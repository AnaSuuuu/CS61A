...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(3):
            ch = opponent.hand[0]
            opponent.hand.remove(ch)
            opponent.draw()
        name = opponent.name
        print(f"{name} discarded and re-drew 3 cards!") 
...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(len(opponent.hand)):
            tmp = opponent.hand[i].attack
            opponent.hand[i].attack = opponent.hand[i].defense
            opponent.hand[i].defense = tmp
        """
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(len(copy)):
            if copy[i].attack == other_card.attack or copy[i].defense == other_card.defense:
                opponent.deck.cards.remove(copy[i])
